...
├── eslint.config.mjs                # create-next-app default (flat config)
├── AGENTS.md                        # generated by create-next-app, left as-is
├── .gitignore
├── package.json
└── README.md (minimal)
```

No `tailwind.config.ts` — see Color system below.
...
**Tailwind v4 has no `tailwind.config.ts`** — `create-next-app` and `shadcn init` both generate CSS-native theming: color tokens live in `globals.css` inside an `@theme` block, and Tailwind auto-generates matching utility classes from them. This project follows that mechanism rather than the `tailwind.config.ts` approach `CLAUDE.md` originally described (that file has been updated to match).

`globals.css`'s `@theme` block defines exactly the CSS variables from `CLAUDE.md`:

- `--color-primary-100/300/DEFAULT/700/900` (blue)
- `--color-accent-100/300/DEFAULT/700/900` (green)
- `--color-surface-0/1/2`, `--color-border`

This auto-generates utility classes: `bg-primary`, `text-primary-700`, `bg-accent-100`, `bg-surface-0`, `border-border`, etc. No raw hex/rgb values appear outside `globals.css`. Default Tailwind color classes remain technically reachable (Tailwind doesn't purge its base palette), but nothing in the scaffold references them — future code review enforces the "no static colors" rule from `CLAUDE.md`.

`shadcn init` also writes its own semantic tokens (`--card`, `--popover`, `--muted`, `--secondary`, `--destructive`, `--ring`, `--input`, `--sidebar-*`, `--chart-*`, `--radius`) that `CLAUDE.md`'s palette doesn't cover. Since no shadcn UI components are added in Phase 1, these are left at shadcn's generated defaults; only `--primary`, and the added `--accent`/`--surface-*`/`--border` tokens, are overridden with `CLAUDE.md`'s values.
